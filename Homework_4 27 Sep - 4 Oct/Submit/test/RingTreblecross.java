...
public class RingTreblecross {
    ...
    **/
    ...
    public Best minValueAndBestSuccessor(GameState state) {
    	...
    	int preMinScore = Integer.MAX_VALUE;
    	GameState bestParent = new GameState(state.getBoard());
    	
    	List<GameState> successors = getSuccessors(state);
    	for (GameState s : successors) {
    		minScore = Integer.min(minScore, maxValueAndBestSuccessor(s).score);
    		if ( minScore < preMinScore) {
    			bestParent = s;
    			preMinScore = minScore;
    		}
    	}
    	
    	return new Best(minScore, bestParent);    	
    }
    ...
    public Best maxValueAndBestSuccessor(GameState state) {
      /* TO DO
    	...
    	int preMaxScore = Integer.MIN_VALUE;
    	GameState bestParent = new GameState(state.getBoard());
    	
    	List<GameState> successors = getSuccessors(state);
    	for (GameState s : successors) {
    		maxScore = Integer.max(maxScore, minValueAndBestSuccessor(s).score);
    		if ( maxScore > preMaxScore) {
    			bestParent = s;
    			preMaxScore = maxScore;
    		}
    	}
    	
    	return new Best(maxScore, bestParent);
    }
    ...
    public List<GameState> getSuccessors(GameState state) {
    	...
    	boolean[] inputBoard = state.getBoard();
    	...
    	if (successors.size()>0) {
    		...
    		System.err.println("ZERO successor states were produced, Please check your input Game State " + state.toString());
    		System.exit(-1);
    	}
    	return null; 
    }
